...
use Scandiweb\StoreFinder\Model\ResourceModel\Event\CollectionFactory as EventCollectionFactory;
...
class Store extends AbstractModel
{
    ...
    public function getEvents($refresh = false)
    {
        if ($refresh || !$this->getData(self::KEY_EVENTS)) {
            if (empty($this->getEventIds())) {
                $this->setData(self::KEY_EVENTS, []);
            } else {
                /** @var EventCollection $eventCollection */
                $eventCollection = $this->eventCollectionFactory->create();
                $eventCollection->addFieldToFilter(EventModel::COLUMN_EVENT_ID, ['IN' => $this->getEventIds()]);

                $this->setData(self::KEY_EVENTS, $eventCollection->getItems());
            }
        }
